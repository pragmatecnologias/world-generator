...
export function generateScatterZones(project: WorldProject, config: WorldGenerationConfig, terrain: TerrainData, roads: { points: { x: number; y: number; z: number }[]; width: number }[]): ScatterGenerationResult {
  ...
  const propAssets = project.assets.filter((asset) => asset.tags.some((tag) => /rock|prop/i.test(tag)) || asset.category.toLowerCase().includes("prop"));
  ...
  const points = [
    { x: -terrain.width * 0.18, y: 0, z: terrain.depth * 0.16 },
    { x: terrain.width * 0.2, y: 0, z: terrain.depth * 0.18 },
  ];
  ...
  for (let i = 0; i < count; i += 1) {
    const x = (rng() - 0.5) * terrain.width * 0.82;
    const z = (rng() - 0.5) * terrain.depth * 0.82;
